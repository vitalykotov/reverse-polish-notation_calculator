...
//
//            if (matcher.matches()) { // число — сразу в результат
//                ans.add(token);
...
//            } else { // оператор
//                while (!stack.isEmpty()
//                        && pri.containsKey(stack.peek().charAt(0))
...
//                    locals.push(result);
//                    System.out.println(String.format("%d) %c%f = %f", counter, c, last, locals.peek()));
//                    continue;
//                }
//
...
//                locals.push(result);
//                System.out.println(String.format("%d) %f %c %f = %f", counter, first, c, second, locals.peek()));
//            }
//        }
//
